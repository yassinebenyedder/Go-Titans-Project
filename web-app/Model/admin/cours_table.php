...
class cours_a_m implements cours_a_m_interface {
 ...
 //create a function to list all courses
    ...
    public function update_cours_m($nomcour,$jour,$horaire,$trainer,$maximale,$id){
        $connection=$this->database_connect();
            try {
          $resultat=$connection->query("UPDATE `cours` SET `nom`='$nomcour',`jour`='$jour',`horaire`='$horaire',`nb max`='$maximale',`trainer_id`='$trainer' WHERE `cour_id`='$id'");
                if(!$resultat){
                    die("error");
                }else{
                    return $resultat;
